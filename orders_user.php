...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	switch ($_POST['action']){
		case "getlist":
            ...
            $sql="SELECT os.*, dy.name, dy.description AS dy_description, dy.price FROM `orders` AS os INNER JOIN `delivery` AS dy WHERE os.statut LIKE '".$status."' AND os.livraison = dy.id AND os.`user` = '".$_SESSION["user"]."' AND os.`name_group` = '".$_SESSION["name_group"]."' ORDER BY os.date_create;";
                $aDatas=db_query($sql);
    ...
    <body ng-controller="appCommon" ng-init="getFromUrl(); getBadgeCart();">
        ...
        <main>
            <div class="flex-column p-5">
                ...
                <div id="toolbar">
                    <a class="btn btn-secondary me-3" href="/amazon.php"><i class="fab fa-amazon me-1"></i>Retourner au catalogue</a>
